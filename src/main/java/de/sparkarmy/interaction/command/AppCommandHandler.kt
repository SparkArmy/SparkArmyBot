package de.sparkarmy.interaction.command

import de.sparkarmy.Config
...
class AppCommandHandler(
    ...
    suspend fun updateGuildCommands(jdaGuild: Guild, force: Boolean = false) {
        ...
        if (!force) {
            ...
            if (!shouldUpdate)
                return log.debug { "Not updating guild commands for ${guild.guildName} (${guild.id}), up to date." }

            log.debug { "Updating guild commands for ${guild.guildName} (${guild.id})" }
        } else {
            log.info { "Force updating guild commands for ${guild.guildName} (${guild.id})" }
        }
