...
public class ModmailCommand extends CustomCommandListener {
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        ...
        if (null == directory){
            this.logger.warn(LoggingMarker.MODMAIL,"directory from modmail is null");
            event.reply("Ups something went wrong").setEphemeral(true).queue();
            return;
        }
        if (!FileHandler.createFile(directory,idExtension + ".json")){
            this.logger.warn(LoggingMarker.MODMAIL,"can't create a file in the modmail-directory");
            event.reply("Ups something went wrong").setEphemeral(true).queue();
            return;
        }
