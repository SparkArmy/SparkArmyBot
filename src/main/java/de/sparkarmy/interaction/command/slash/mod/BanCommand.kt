...
class BanCommand : SlashCommand("ban", "Ban a user from the server") {
    ...
    suspend fun run(event: SlashCommandInteractionEvent, user: User, reason: String, messages: Boolean = false) {
        ...
        checkPreconditions(user, event.member, guild)
            .flatMap {
                when {
                    it -> guild.ban(user, duration, TimeUnit.DAYS)
                        .reason(reason)
                        .flatMap {
                            hook.editOriginal(
                        ...
                        .onErrorFlatMap(
                            ...
                            hook.editOriginal(
                                event.getLocalizedString(
                                    "commands.punishment.ban.userBanFailed",
                                    false,
                                    user.effectiveName
                                )
                            )
                    ...
                    else -> event.reply(event.getLocalizedString("commands.punishment.ban.userBanFailed", true, user))
                }
            }
            .await()
    }
